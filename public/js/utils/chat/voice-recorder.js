/**
 * Voice Recorder Utility Module for Wapi Chat
 * Handles MediaRecorder stream capturing, live frequency visualization on Canvas,
 * recording timer intervals, pause/resume, and recording submission.
 */
...
window.drawSyntheticVisualizer = function () {
    ...
    function renderFrame() {
        ...
        for (let i = 0; i < numBars; i++) {
            ...
            const h = s.isPausedRecording ? 4 : Math.max(3, val * canvas.height * 0.85);
            const x = i * (barWidth + gap);
            const y = (canvas.height - h) / 2;
...
window.pauseRecording = function () {
    ...
    if (s.isPausedRecording) {
        ...
        if (s.mediaRecorder && s.mediaRecorder.state === 'recording') s.mediaRecorder.pause();
    } else {
        pauseBtn.querySelector('.pause-icon').style.display = 'block';
        pauseBtn.querySelector('.resume-icon').style.display = 'none';
        if (s.mediaRecorder && s.mediaRecorder.state === 'paused') s.mediaRecorder.resume();
    }
};
...
window.stopRecording = function (discard = false) {
    ...
    window.messageMetaData.set(newMsgId, { duration: durationSec, speed: 1, peaks });
